...
- **#33** Business-claims-verifiering FÖRBJUDET att fabricera: Innan ANY UI-text/marketing-copy/email-template/hero-banner committas måste jag (a) sökt `docs/sanning/approved-claims.json` att claim är godkänd, (b) sökt `explicitly_disallowed_claims`-listan att text inte bryter (ex: "30/60/90 dagars betalvillkor", "trappsystem", "alltid svenska städare", "Sveriges största"). Vid tvivel → fråga Farhad EXPLICIT innan commit. Lärdom 2026-04-27: hero-banner på boka.html innehöll fabricerad "månadsdebitering, dedikerade städare, avtal" som inte finns i Spicks affärsmodell — fångad av Farhad i prod, inte av min process. Detta får aldrig hända igen.
- **#34** Visuell verifiering EFTER commit: efter UI-commit MÅSTE jag köra `preview_screenshot`/`preview_snapshot` (eller curl/SQL för backend) för att se RENDRAD output med live-data. `preview_eval` verifierar bara att DOM-element + JS-funktion **definierats**, INTE att de **rendrar rätt**. Om jag inte kan verifiera live (kräver auth/specifik data) → be Farhad skicka screenshot omedelbart, vänta inte tills han granskar dagen efter. Lärdom 2026-04-28: Clara-Maria-kortet visade '09:30' istället för '11:30' (TZ-bug) + '773+772=1545' istället för 1544 (rounding-bug) — båda hade fångats av visuell verify innan Farhads granskning.
